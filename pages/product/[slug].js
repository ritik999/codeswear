        ...
        <section className="text-gray-600 body-font overflow-hidden">
          <div className="container px-5 py-12 mx-auto">
            <div className="lg:w-4/5 mx-auto flex flex-wrap">
              <img alt="ecommerce" className="lg:w-1/2 md:h-[70vh] w-full h-64 object-contain object-top rounded" src={product.img} />
              <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                ...
                <div className="flex">
                  ...
                  <button onClick={()=>buyNow(slug, 1, 499, product.title, product.size, product.color)} className="flex ml-6 text-white bg-pink-500 border-0 py-2 px-1 md:px-6 focus:outline-none hover:bg-pink-600 rounded text-sm md:text-lg">Buy now</button>
                  <button onClick={() => addToCart(slug, 1, 499, product.title, product.size, product.color)} className="flex ml-2 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded text-sm md:text-lg">Add to cart</button>
...
export async function getServerSideProps(context) {
  ...
  let product = await ProductSchema.findOne({ slug: context.query.slug });
  let variants = await ProductSchema.find({ title: product.title, category:product.category })
  let colorSizeSlug = {}
