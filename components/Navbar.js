...
const Navbar = ({logout, user, cart, addToCart, removeFromCart, clearCart, subTotal }) => {
    ...
    return (
        <div className='flex flex-col justify-center items-center md:flex-row md:justify-start py-2 shadow-md sticky top-0 z-10 bg-white'>
            ...
            <div onMouseOver={() => setDropdown(true)} onMouseLeave={() => setDropdown(false)} className='cart absolute right-0 top-4 mx-5 flex items-center cursor-pointer'>
                <div onMouseOver={() => setDropdown(true)} onMouseLeave={() => setDropdown(false)}>
                    {dropdown && <div  className='absolute top-5 right-8 bg-white shadow-md py-4 px-4 rounded-md w-32'>
            ...
            <div ref={ref} className='w-72 h-[100vh] sideCart absolute top-0 right-0 bg-pink-100 p-10 transform translate-x-full transition-transform'>
                ...
                <div className='flex mt-5'>
                    <Link href={'/checkout'}><button className="flex mx-auto text-white bg-pink-500 border-0 py-2 px-2 focus:outline-none hover:bg-pink-600 rounded text-md"><BsFillBagCheckFill className='m-1' />Checkout</button></Link>
                    <button onClick={clearCart} className="flex mx-auto text-white bg-pink-500 border-0 py-2 px-2 focus:outline-none hover:bg-pink-600 rounded text-md">Clear Cart</button>
                </div>
            </div>
        </div>
    )
